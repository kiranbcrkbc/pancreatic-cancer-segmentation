"""
Training Pipeline for Model 1: CNN + Pyramid Transformer (CNNPyramidTransformerSeg)

Provides a clean, standalone training entry point with configurable CLI arguments.
Supports 5-fold cross-validation or single-split training with Compound Loss
(Soft Dice + Weighted Cross-Entropy + Focal Loss) and AdamW optimizer.

Usage:
    python train.py --epochs 10 --batch-size 16 --lr 0.001
...
# Import standalone model architecture
...
class CompoundLoss(nn.Module):
    """
    Project Compound Loss:
    L = dice_w * Dice + ce_w * Weighted_CE + focal_w * Focal
    Default weights: 0.60 Dice, 0.30 Cross-Entropy, 0.10 Focal
    """
...
class SyntheticPatchDataset(Dataset):
    ...
    def __getitem__(self, idx: int) -> tuple[torch.Tensor, torch.Tensor]:
        # Generate normalized CT patch in [0, 1]
        x = torch.rand(1, *self.patch_size, dtype=torch.float32)
        # Generate target mask with labels {0: bg, 1: pancreas, 2: tumor}
...
def train_model(
    ...
    print(f"=== Initializing Model 1: CNN + Pyramid Transformer ===")
    ...
    criterion = CompoundLoss(dice_w=0.60, ce_w=0.30, focal_w=0.10).to(device_obj)
    ...
    for epoch in range(1, num_epochs + 1):
        ...
        with torch.no_grad():
            for images, masks in val_loader:
                ...
                # Compute foreground Dice (Classes 1 and 2)
                for c in [1, 2]:
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Train Model 1: CNN + Pyramid Transformer")
