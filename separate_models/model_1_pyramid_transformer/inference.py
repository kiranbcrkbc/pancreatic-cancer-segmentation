"""
Inference & Interpretability Pipeline for Model 1: CNN + Pyramid Transformer
...
Preprocessing (HU windowing [-150, +250], Min-Max scaling [0.0, 1.0], Resizing)
    ↓
Model Forward Pass (CNNPyramidTransformerSeg)
    ↓
Segmentation Prediction (Argmax -> 0: Background, 1: Pancreas, 2: Tumor)
    ↓
Grad-CAM Interpretability (Class 2: Pancreatic Tumor Activation Map)
    ↓
Saved Output Masks & Diagnostic Panels

Usage:
    python inference.py --checkpoint ../../checkpoints/pyramid/final_model.pt --output-dir ./outputs
    python inference.py --input /path/to/slice.png --checkpoint ../../checkpoints/pyramid/final_model.pt
"""
...
from model import CNNPyramidTransformerSeg
...
class CTPreprocessor:
    ...
    def preprocess_slice(self, slice_array: np.ndarray) -> np.ndarray:
        """Clips HU window, normalizes to [0, 1], and resizes to target dimension."""
        # 1. HU Windowing
...
class GradCAM:
    ...
    def generate_heatmap(self, input_tensor: torch.Tensor, target_class: int = 2) -> np.ndarray:
        ...
        # Score is the spatial sum of logits for the specified class
...
def resolve_checkpoint(path_str: str) -> Path:
    ...
    if cand1.exists():
        return cand1
    cand2 = (script_dir.parent.parent / "checkpoints" / "pyramid" / p.name).resolve()
    if cand2.exists():
        return cand2
    return p
...
def run_inference(
    ...
    checkpoint_path: str = "../../checkpoints/pyramid/final_model.pt",
    ...
    print(f"=== Model 1: CNN + Pyramid Transformer Inference ===")
    print(f"Device: {device_obj}")

    model = CNNPyramidTransformerSeg(in_channels=1, num_classes=3).to(device_obj)
    ...
    # Load or generate input CT slice
    if input_path and Path(input_path).exists():
        ...
        # Create synthetic CT patch with anatomical contours
        raw_slice = np.ones((128, 128), dtype=np.float32) * -100.0  # Background HU
        y, x = np.ogrid[:128, :128]
        pancreas_mask = ((x - 64) ** 2 / 30**2 + (y - 64) ** 2 / 16**2) <= 1
        tumor_mask = ((x - 70) ** 2 / 8**2 + (y - 64) ** 2 / 8**2) <= 1
        raw_slice[pancreas_mask] = 40.0   # Soft tissue HU
        raw_slice[tumor_mask] = 80.0      # Enhanced tumor HU
    ...
    plt.suptitle("Model 1 (CNN + Pyramid Transformer) Clinical Diagnostic Inference", fontsize=13, fontweight="bold")
    plt.tight_layout()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Inference for Model 1: CNN + Pyramid Transformer")
    parser.add_argument("--input", type=str, default=None, help="Path to input CT slice image or .npy file")
    parser.add_argument("--checkpoint", type=str, default="../../checkpoints/pyramid/final_model.pt", help="Path to trained checkpoint")
