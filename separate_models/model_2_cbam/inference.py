"""
Inference & Interpretability Pipeline for Model 2: CNN + CBAM (CBAMNet)
...
def resolve_checkpoint(path_str: str) -> Path:
    ...
    if cand1.exists():
        return cand1
    cand2 = (script_dir.parent.parent / "checkpoints" / "cbam" / p.name).resolve()
    if cand2.exists():
        return cand2
    return p
...
def run_inference(
    ...
    checkpoint_path: str = "../../checkpoints/cbam/final_model.pt",
    ...
    print(f"=== Model 2: CNN + CBAM Inference ===")
    print(f"Device: {device_obj}")

    model = CBAMNet(in_channels=1, num_classes=3).to(device_obj)
    ...
    plt.suptitle("Model 2 (CNN + CBAM) Clinical Diagnostic Inference", fontsize=13, fontweight="bold")
    plt.tight_layout()
...
def main() -> None:
    parser = argparse.ArgumentParser(description="Inference for Model 2: CNN + CBAM")
    parser.add_argument("--input", type=str, default=None, help="Path to input CT slice image or .npy file")
    parser.add_argument("--checkpoint", type=str, default="../../checkpoints/cbam/final_model.pt", help="Path to trained checkpoint")
